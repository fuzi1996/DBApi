...
import { login, loginParam } from '/@/api/user/index'
...
export const useLayoutStore = defineStore({
  ...
  actions: {
    ...
    removeActiveTag() {
      const index = this.tags.tagsList.findIndex(v => v.isActive)
      this.tags.tagsList.splice(index,1)
      this.tags.cachedViews.splice(index,1)
    },
    // 添加缓存页面
    ...
    getToken():string {
      return this.status.ACCESS_TOKEN
    },
    ...
    async login(param: loginParam):Promise<string> {
      ...
      if(token) {
        this.userInfo.name = param.username  
        this.status.ACCESS_TOKEN = token
        setLocal('token', this.status, 1000 * 60 * 60)
