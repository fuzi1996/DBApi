...
import { IGenerateCallExampleCodeParam,IDetail,IGenerateParam,GetEffectValueFunction } from '/@/type/utils/code'

const getEffectValue:GetEffectValueFunction = (type:string, value, expectValue):string|number => {
  switch (type) {
  case DATA_TYPE.STRING:
    return typeof value === 'string' ? value : expectValue || ''
  case DATA_TYPE.BIGINT:
    return !isNaN(value as number) ? value : expectValue || 0
  case DATA_TYPE.DOUBLE:
    return !isNaN(value as number) ? value : expectValue || 1.0
  case DATA_TYPE.DATE:
...
function generateParam({ lang, address, detail }:IGenerateCallExampleCodeParam):IGenerateParam {
  const { contentType } = detail
  const { params } = detail
  const requestUrl = address
  const { previlege } = detail
...
export function generateCallExampleCode({ lang, address, detail }:IGenerateCallExampleCodeParam):string {
