...
export function generateJavaCallExampleCode(param:IGenerateParam):string {
...
function generatePrarmCode(params:IParamItem[], getEffectValue:GetEffectValueFunction):string[] {
  const codes:string[] = []
  params.forEach(item => {
    ...
    let value = getEffectValue(itemType, item.value as string|number)
    ...
    case DATA_TYPE.BIGINT:
      value = getEffectValue(itemType, item.value as string|number, '0L')
      codes.push(`params.put("${itemName}", ${value});`)
      break
    case DATA_TYPE.DOUBLE:
      value = getEffectValue(itemType, item.value as string|number, '1D')
      codes.push(`params.put("${itemName}", ${value});`)
      break
    case DATA_TYPE.DATE:
      codes.push(`params.put("${itemName}", "${value}");`)
